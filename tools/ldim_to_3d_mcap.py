...
def main():
    ...
    ap.add_argument("--merge", action="store_true", help="merge all scans into one frame")
    ...
    args = ap.parse_args()
    out = args.output or args.input.rsplit(".", 1)[0] + ("_static.mcap" if args.merge else "_3d.mcap")
    ...
    if args.merge:
        ...
        merged = np.column_stack([x, y, z, inten])
        t0 = packets[0][0]
        print(f"Writing {len(merged)} points to {out}...")
        with open(out, 'wb') as f:
            w = Ros2Writer(f)
            schema = w.register_msgdef('sensor_msgs/msg/PointCloud2', POINT_CLOUD2_SCHEMA)
            msg = build_pc2(merged, t0)
            w.write_message(topic="/points", schema=schema, message=msg, log_time=t0, publish_time=t0)
            w.finish()
    else:
